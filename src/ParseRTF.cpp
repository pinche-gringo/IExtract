...
auto value = [](auto &ctx) {
  State &st(state(ctx));
  TRACE8("ParseRTF::parse (std::istream&, Properties&) - Value: "
         << x3::_attr(ctx));
  if (st.entry)
...
auto const entry = x3::lit('{') >> ws >>
                   (x3::lit("\\author")[author] | x3::lit("\\title")[title] |
                    x3::lit("\\doccomm")[comment] | otherCmd) >>
                   ws >> text[value] >> x3::lit('}') >> ws;
auto const info = x3::lit("\\info") >> ws >> +entry >>
                  x3::lit('}')[finish] >> ws;

x3::rule<class CommandsID> const commands("RTF commands");
x3::rule<class BlockID> const block("RTF block");

auto const commands_def = *(x3::eps[notDone] >> (info | block | otherCmd |
                                                 (+~x3::char_("\\{}") >> ws)));
...
void ParseRTF::parse(std::istream &stream, Properties &result) {
  ...
  SpiritParser::parse(SpiritParser::readStream(stream),
                      x3::with<StateTag>(std::ref(st))[document],
                      _("RTF document"));
}
