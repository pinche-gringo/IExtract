...
void ParseHTML::parse(std::istream &stream, Properties &result) {
  ...
  auto metaName = [&entry](auto &ctx) {
    ...
    entry = (((name == "description") || (name == "dc.description"))
                 ? &Properties::strComment
             : ((name == "author") || (name == "dc.creator"))
  ...
  auto tagScript = nocase("script") >> restOfTag >> x3::lit('>') >> ws >>
                   x3::omit[*(x3::char_ - nocase("</script"))] >>
                   nocase("</script") >> restOfTag;

  auto tag = x3::lit('<') >> ws >>
             (tagTitle | tagMeta | tagEndHead | tagScript | restOfTag) >>
             x3::lit('>') >> ws;
  auto document =
      *(x3::eps[notEndOfHead] >> (tag | x3::omit[+~x3::char_('<')]));
