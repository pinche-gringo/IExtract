...
void ParseJPEG::parse(std::istream &stream, Properties &result) {
  ...
  auto hasLength = [&marker](auto &ctx) {
    x3::_pass(ctx) =
        (marker != M_TEM) && ((marker < M_RST0) || (marker > M_RST7));
  };
  ...
  auto segment = [&](auto &ctx) {
    ...
    TRACE8("ParseJPEG::parse (std::istream&, Properties&) - Marker "
           << std::hex << marker << std::dec << ": " << data.size()
           << " bytes");
    switch (marker) {
    case M_COM:
  ...
  auto markerData =
      x3::eps[hasLength] >> x3::big_word[setLength] >> bytes(length)[segment];
  auto image =
      x3::lit("\xff\xd8") >> *(markerID >> !x3::eps[isEnd] >> -markerData);
  ...
  auto parseIFD = [&tiff, &result](auto word, auto dword) {
    ...
    auto entry = [&](auto &ctx) {
      static const std::pair<unsigned int, std::string Properties::*> tags[] = {
          {TAG_TITLE, &Properties::strTitle},
          {TAG_COMMENT, &Properties::strComment},
          {TAG_AUTHOR, &Properties::strAuthor}};

      // The value is stored in the entry, if it fits into the 4 bytes of the
      // offset
      std::size_t posValue((cValues > 4)
                               ? static_cast<std::size_t>(x3::_attr(ctx))
                               : static_cast<std::size_t>(
                                     x3::_where(ctx).begin() - tiff.data()));
      TRACE8("ParseJPEG::parseEXIF (const std::string&, Properties&) - Tag "
             << std::hex << tag << std::dec << " (" << type << "): " << cValues
             << " @ " << posValue);
      if (type != TYPE_BYTE)
        return;
    ...
    SpiritParser::parse(act, tiff.data() + tiff.size(),
                        x3::repeat(2)[x3::byte_] >> word >>
                            dword[setValue(offset)],
                        _("TIFF header"));
    if (offset >= tiff.size())
    ...
    SpiritParser::parse(
        ...
        word[setValue(count)] >>
            SpiritParser::times(
                count)[word[setValue(tag)] >> word[setValue(type)] >>
...
void ParseJPEG::parsePhotoshop(const std::string &data, Properties &result) {
  ...
  auto resource = [&id, &result](auto &ctx) {
    TRACE8("ParseJPEG::parsePhotoshop (const std::string&, Properties&) - "
           "Resource "
           << std::hex << id << std::dec << ": " << x3::_attr(ctx).size()
           << " bytes");
    if (id == RES_IPTC)
  ...
  auto block = x3::lit("8BIM") >> x3::big_word[setID] >>
               ...
               x3::big_dword[setLength] >> bytes(length)[resource] >>
               skip(padding);
...
void ParseJPEG::parseIPTC(const std::string &data, Properties &result) {
  ...
  auto value = [&](auto &ctx) {
    TRACE8("ParseJPEG::parseIPTC (const std::string&, Properties&) - "
           << record << ':' << dataset << " = " << x3::_attr(ctx));
    if (record != 2)
      return;
    switch (dataset) {
    case IPTC_TITLE:
  ...
  auto dataSet = x3::lit('\x1c') >> x3::byte_[setRecord] >>
                 x3::byte_[setDataset] >> x3::big_word[setLength] >>
                 bytes(length)[value];
