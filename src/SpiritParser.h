...
template <bool Store> struct Bytes : x3::parser<Bytes<Store>> {
  using attribute_type =
      std::conditional_t<Store, std::string, x3::unused_type>;
  static constexpr bool has_attribute = Store;

  /// Constructor
  /// \param count Reference to the number of bytes to consume; evaluated while
  /// parsing
  explicit Bytes(const std::size_t &count) : count(count) {}
  Bytes(std::size_t &&) = delete;

  template <typename It, typename Context, typename RContext,
            typename Attribute>
  bool parse(It &first, const It &last, const Context &, RContext &,
...
inline Bytes<true> bytes(const std::size_t &count) {
  return Bytes<true>(count);
}
/// Returns a parser consuming (and ignoring) \c count bytes
inline Bytes<false> skip(const std::size_t &count) {
  return Bytes<false>(count);
}
...
struct Times : x3::unary_parser<Subject, Times<Subject>> {
  ...
  Times(const Subject &subject, const std::size_t &count)
      : base_type(subject), count(count) {}

  template <typename It, typename Context, typename RContext,
            typename Attribute>
  bool parse(It &first, const It &last, const Context &context,
             RContext &rcontext, Attribute &) const {
    It start(first);
...
struct TimesGen {
  ...
  template <typename Subject>
  Times<typename x3::extension::as_parser<Subject>::value_type>
...
void parse(Iterator &first, Iterator last, const Parser &parser,
...
/// \throw YGP::ParseError If the data does not match the parser
template <typename Parser>
void parse(const std::string &data, const Parser &parser,
           const char *description) {
