...
template <typename Subject> struct Times : x3::unary_parser<Subject, Times<Subject>> {
...
template <typename Parser> void parse(Iterator& first, Iterator last, const Parser& parser, const char* description) {
    bool ok(false);
    try {
...
/// \param description Description of the parsed data (for error messages)
/// \throw YGP::ParseError If the data does not match the parser
template <typename Parser> void parse(const std::string& data, const Parser& parser, const char* description) {
