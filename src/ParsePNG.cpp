...
void ParsePNG::parse(std::istream &stream) {
  ...
  auto chunkData = [this, &type](auto &ctx) {
    foundChunk(type, x3::_attr(ctx));
  };
  ...
  auto image =
      x3::lit("\x89PNG\x0d\x0a\x1a\x0a") >> x3::repeat(2, x3::inf)[chunk];
...
void ParsePNG::foundChunk(unsigned int type, const std::string &data) {
  TRACE8("ParsePNG::foundChunk (unsigned int, const std::string&) - "
         << std::hex << type << std::dec << ": " << data.size() << " bytes");
  if (type != TYPE_TEXT)
    return;
