...
void ParseOpenOffice::parse(std::istream& stream, Properties& result) {
    ...
    SpiritParser::parse(act, end,
                        x3::lit(ID_ENDCDR) >> x3::repeat(6)[x3::byte_] >> x3::little_word[setEntries] >>
                            x3::repeat(4)[x3::byte_] >> x3::little_dword[setOffset],
    ...
    auto entry = x3::lit(ID_CFILEHDR) >> x3::repeat(6)[x3::byte_] >> x3::little_word[([&](auto& ctx) {
                     if (!found)
                         method = x3::_attr(ctx);
                 })] >>
                 ...
                 x3::repeat(4)[x3::byte_] >> x3::little_word[set(lenName)] >> x3::little_word[set(lenExtra)] >>
                 x3::little_word[set(lenComment)] >> x3::repeat(8)[x3::byte_] >> x3::little_dword[([&](auto& ctx) {
                     if (!found)
    ...
    TRACE5("ParseOpenOffice::parse (std::istream&, Properties&) - Meta-info at " << posEntry << "; " << size << " bytes");
    ...
    SpiritParser::parse(act, end,
                        x3::lit(ID_LOCALHDR) >> x3::repeat(22)[x3::byte_] >> x3::little_word[set(lenName)] >>
                            x3::little_word[set(lenExtra)] >> skip(lenName) >> skip(lenExtra) >>
...
void ParseOpenOffice::parseMetaInfo(const std::string& metaInfo, Properties& result) {
    ...
    auto element = x3::lit('<') >> (*~x3::char_('>'))[tag] >> x3::lit('>') >> ws >> (*~x3::char_('<'))[value];
