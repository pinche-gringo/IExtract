...
// CREATED     : 8.10.2002
// COPYRIGHT   : Copyright (C) 2002 - 2008, 2024, 2026
...
void ParseOOXML::parse(std::istream& stream, Properties& result) {
    ...
    posFile = YGP::getFileOffsetInArchive(dynamic_cast<std::ifstream&>(stream), reinterpret_cast<char*>(&posFile),
    ...
    if ((YGP::get4BytesLSB(output.data()) != ID_PKZIP_LOCALHDR) || (YGP::get2BytesLSB(output.data() + 8) != 8) ||
    ...
    TRACE9("ParseOOXML::parse (std::istream&, Properties&) - Skipping " << YGP::get2BytesLSB(output.data() + 28));
