...
void ParseOOXML::parse(std::istream &stream, Properties &result) {
  ...
  posFile = YGP::getFileOffsetInArchive(dynamic_cast<std::ifstream &>(stream),
                                        reinterpret_cast<char *>(&posFile),
                                        "docProps/core.xml", 17);
  if (!posFile)
    throw YGP::ParseError(_("Not an OOXML document!"));

  TRACE1("ParseOOXML::parse (std::istream&, Properties&) - Skipping to "
         << posFile);
  stream.seekg(posFile, std::ios::beg);

  std::array<char, 1024> output;
  stream.read(output.data(), 30);
  TRACE9("Hdr: " << YGP::get4BytesLSB(output.data())
                 << "; Compr: " << YGP::get2BytesLSB(output.data() + 8)
  ...
  const std::unique_ptr<z_stream, decltype(&inflateEnd)> cleanup(&zStream,
                                                                 &inflateEnd);

  std::array<char, 1024> input;
  while (stream && size) {
    stream.read(input.data(), (size > input.size()) ? input.size() : size);
    TRACE6("ParseOOXML::parse (std::istream&, Properties&) - Bytes read: "
           << stream.gcount());
    ...
    do {
      ...
      case Z_DATA_ERROR:
        throw YGP::ParseError(
            _("Error inflating stream: Not a deflated file!"));
    ...
    if (rc == Z_STREAM_END) {
      static const std::pair<std::string, std::string Properties::*> entries[] =
          {{"dc:title>", &Properties::strTitle},
           {"dc:creator>", &Properties::strAuthor},
