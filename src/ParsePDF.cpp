...
auto storeOther = [](auto &ctx) {
  ...
  if (st.depth == 1)
    st.others[st.key] =
        std::string(x3::_attr(ctx).begin(), x3::_attr(ctx).end());
};
...
auto const name = x3::rule<class NameID, std::string>("Name") =
    x3::lit('/') >> *regular;
...
// characters
auto const content =
    *(x3::lit('\\') >> (x3::repeat(1, 3)[x3::char_('0', '7')][appendOctal] |
                        x3::lit("\r\n") | x3::lit('\r') | x3::lit('\n') |
                        x3::char_[appendEscaped]) |
      nested[append] | (~x3::char_("()\\"))[append]);
auto const literal_def = x3::lit('(') >> content >> x3::lit(')');
auto const nested_def = x3::char_('(')[append] >> content >>
                        x3::char_(')')[append];
auto const hex_def = x3::lit('<') >> !x3::lit('<') >>
                     (*~x3::char_('>'))[decodeHex] >> x3::lit('>');

auto const value =
    literal[storeString] | hex[storeString] | x3::raw[reference][storeOther] |
    x3::raw[number][storeOther] | x3::raw[name][storeOther] | object;
auto const dictionary_def =
    x3::lit("<<")[enter] >> ws >>
    *(name[setKey] >> ws >> value >> ws) >> x3::lit(">>")[leave];
auto const array_def = x3::lit('[') >> ws >> *(object >> ws) >> x3::lit(']');
auto const object_def = dictionary | array | x3::omit[literal] | x3::omit[hex] |
                        reference | number | x3::omit[name] | keyword;
...
void ParsePDF::parse(std::istream &stream, Properties &result) {
  ...
  if (pos == std::string::npos)
    throw YGP::ParseError(
        _("Document does not contain the offset of the cross reference table"));

  std::size_t offset(0);
  SpiritParser::Iterator act(data.data() + pos);
  SpiritParser::parse(
      act, data.data() + data.size(),
      x3::lit("startxref") >> ws >>
  ...
  while (!(infoFound && (offsets.find(info) != offsets.end()))) {
    ...
    if (trailer.others.find("Prev") == trailer.others.end())
      throw YGP::ParseError(
          _("Document does not contain neither an /Info nor a /Prev entry"));
  ...
  static const std::pair<const char *, std::string Properties::*> entries[] = {
      {"Title", &Properties::strTitle},
      {"Author", &Properties::strAuthor},
...
//-----------------------------------------------------------------------------
ParsePDF::Dictionary
ParsePDF::parseXRefTable(const std::string &data, std::size_t offset,
                         std::map<unsigned int, std::size_t> &offsets) {
  TRACE5(
      "ParsePDF::parseXRefTable (const std::string&, std::size_t, std::map&) - "
      << offset);
  if (offset >= data.size())
...
//-----------------------------------------------------------------------------
ParsePDF::Dictionary ParsePDF::parseObject(const std::string &data,
                                           std::size_t offset,
                                           unsigned int id) {
  TRACE5(
      "ParsePDF::parseObject (const std::string&, std::size_t, unsigned int) - "
      << id << " @ " << offset);
  if (offset >= data.size())
  ...
  auto obj = x3::uint_[checkID] >> ws >> x3::uint_ >> ws >> x3::lit("obj") >>
             ws >> dictionary;

  SpiritParser::Iterator act(data.data() + offset);
  SpiritParser::parse(act, data.data() + data.size(),
                      x3::with<StateTag>(std::ref(st))[obj], _("info object"));
  return values;
}
...
unsigned int ParsePDF::parseReference(const std::string &reference) {
  ...
  SpiritParser::parse(reference,
                      x3::uint_[([&id](auto &ctx) { id = x3::_attr(ctx); })] >>
                          ws >> x3::uint_ >> ws >> x3::lit('R'),
