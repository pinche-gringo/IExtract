...
static constexpr std::string_view SECTIONID ("\x01\0\0\0\xE0\x85\x9F\xF2\xF9\x4F"
                                             "\x68\x10\xAB\x91\x08\0\x2B\x27\xB3\xD9", 0x14);
...
void ParseMSOffice::parse (std::istream& stream, Properties& result) {
   ...
   TRACE6 ("ParseMSOffice::parse (std::istream&, Properties&) - Properties: " << std::hex
   ...
   while (true) {
      for (; entry < (block.data () + lenBlock); entry += 0x80) {
         TRACE9 ("ParseMSOffice::parse (std::istream&, Properties&) - Entry with " << std::hex << YGP::get2BytesLSB (entry + 0x40) << " bytes");
         ...
         TRACE8 ("ParseMSOffice::parse (std::istream&, Properties&) - Info in " << std::hex
                 << offBlock << std::dec << "; " << length << " bytes");

         const unsigned int cBlocks ((length & (lenBlock - 1))
                                     ? ((length >> sizeBlock) + 1) : (length >> sizeBlock));
         ...
         if (length < 4096) {
            ...
            while (length > 0) {
               ...
               std::memcpy (actPos, pos->second.data () + ((offBlock % cSmallBlocks) << sizeBlockSmall),
                            lenSmallBlock);
         ...
         // Check info-block
         TRACE9 ("ParseMSOffice::parse (std::istream&, Properties&) - InfoSummary: " << std::hex << YGP::get2BytesLSB (infoBlock.data ()) << std::dec);
         ...
         TRACE9 ("ParseMSOffice::parse (std::istream&, Properties&) - Start section: " << std::hex << YGP::get4BytesLSB (infoBlock.data () + 0x2c) << std::dec);
         ...
         for (entry = actPos + 8; entries--; entry += 8) {
            ...
            TRACE7 ("ParseMSOffice::parse (std::istream&, Properties&) - Entry: " << YGP::get4BytesLSB (entry) << ": Offset " << std::hex << offset << std::dec);
            ...
            case TYPE_TITLE: value = &Properties::strTitle; break;
            case TYPE_AUTHOR: value = &Properties::strAuthor; break;
            case TYPE_COMMENT: value = &Properties::strComment; break;
            } // end-switch
            if (value) {
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
void ParseMSOffice::readBlock (std::istream& stream, unsigned int offBlock, char* block,
			       unsigned int sizeBlock) {
   TRACE9 ("ParseMSOffice::readBlock (std::istream&, unsigned int, char*, unsigned int): " << offBlock);
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
ParseMSOffice::BAT ParseMSOffice::readBAT (std::istream& stream, const char* pBATBlocks,
   ...
   for (unsigned int i (0); i < cBlocks; ++i)
      readBlock (stream, YGP::get4BytesLSB (pBATBlocks + (i << 2)),
                 reinterpret_cast<char*> (bat.data ()) + (i << sizeBlock), sizeBlock);
   return bat;
}
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
std::vector<char> ParseMSOffice::readFile (std::istream& stream, unsigned int offBlock, const BAT& bat,
