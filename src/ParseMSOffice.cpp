...
static constexpr std::string_view
    SUMMARY("\x05\0\x53\0\x75\0\x6D\0\x6D\0\x61\0\x72\0\x79\0"
            "\x49\0\x6E\0\x66\0\x6F\0\x72\0\x6D\0\x61\0\x74\0"
            "\x69\0\x6F\0\x6E\0\0",
            0x28);
static constexpr std::string_view ROOTENTRY("R\0o\0o\0t\0 \0E\0n\0t\0r\0y\0\0",
                                            0x16);
static constexpr std::string_view
    SECTIONID("\x01\0\0\0\xE0\x85\x9F\xF2\xF9\x4F"
...
static constexpr std::uint32_t LAST_BLOCK =
    0x80000000; // Block-indices above are special
...
void ParseMSOffice::parse(std::istream &stream, Properties &result) {
  ...
  TRACE8("ParseMSOffice::parse (std::istream&, Properties&) - Sizes: "
         << (1 << sizeBlock) << '/' << (1 << sizeBlockSmall));
  ...
  TRACE6("ParseMSOffice::parse (std::istream&, Properties&) - Properties: "
         << std::hex << offProperties << " -> "
  ...
  if ((YGP::get2BytesLSB(block.data() + 0x40) != ROOTENTRY.size()) ||
    ...
    error.replace(error.find("%1"), 2,
                  std::format("{:X}", (offProperties << sizeBlock) + 512));
  ...
  while (true) {
    for (; entry < (block.data() + lenBlock); entry += 0x80) {
      ...
      if ((YGP::get2BytesLSB(entry + 0x40) != SUMMARY.size()) ||
          (std::string_view(entry, SUMMARY.size()) != SUMMARY))
        continue;

      std::int32_t length(YGP::get4BytesLSB(entry + 0x78));
      std::uint32_t offBlock(YGP::get4BytesLSB(entry + 0x74));
      TRACE8("ParseMSOffice::parse (std::istream&, Properties&) - Info in "
             << std::hex << offBlock << std::dec << "; " << length << " bytes");

      const unsigned int cBlocks((length & (lenBlock - 1))
                                     ? ((length >> sizeBlock) + 1)
                                     : (length >> sizeBlock));
      ...
      if (length < 4096) {
        // Read SBAT table
        const std::vector<char> dataSBAT(
            readFile(stream, YGP::get4BytesLSB(header.data() + 0x3c), bat,
                     YGP::get4BytesLSB(header.data() + 0x40), sizeBlock));
        BAT sbat(dataSBAT.size() / sizeof(BAT::value_type));
        std::memcpy(sbat.data(), dataSBAT.data(),
                    sbat.size() * sizeof(BAT::value_type));
        ...
        while (length > 0) {
          const unsigned int bigBlock(
              getBlock(bat, offSBA, offBlock >> (sizeBlock - sizeBlockSmall)));
          ...
          std::memcpy(actPos,
                      pos->second.data() +
                          ((offBlock % cSmallBlocks) << sizeBlockSmall),
                      lenSmallBlock);
      ...
      if ((YGP::get2BytesLSB(infoBlock.data()) != 0xfffe) ||
          (std::string_view(infoBlock.data() + 0x18, SECTIONID.size()) !=
           SECTIONID))
        throw YGP::ParseError(_("Not an Information-Summary section!"));

      TRACE9(
          "ParseMSOffice::parse (std::istream&, Properties&) - Start section: "
          << std::hex << YGP::get4BytesLSB(infoBlock.data() + 0x2c)
          << std::dec);
      const char *actPos(infoBlock.data() +
                         YGP::get4BytesLSB(infoBlock.data() + 0x2c));
      unsigned int entries(YGP::get4BytesLSB(actPos + 4));
      TRACE3("ParseMSOffice::parse (std::istream&, Properties&) - Entries: "
             << entries);
      if (entries > 50)
        throw YGP::ParseError(
            _("Number of information entries not plausible!"));

      for (entry = actPos + 8; entries--; entry += 8) {
        const std::uint32_t offset(YGP::get4BytesLSB(entry + 4));
        TRACE7("ParseMSOffice::parse (std::istream&, Properties&) - Entry: "
               << YGP::get4BytesLSB(entry) << ": Offset " << std::hex << offset
               << std::dec);

        if ((offset >= YGP::get4BytesLSB(actPos)) ||
            ((YGP::get4BytesLSB(actPos + offset) == 0x1e)
                 ? ((YGP::get4BytesLSB(actPos + offset + 4) >=
                     YGP::get4BytesLSB(actPos)) ||
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
void ParseMSOffice::readBlock(std::istream &stream, unsigned int offBlock,
  ...
  if (!stream) {
    ...
    error.replace(error.find("%1"), 2,
                  std::format("{:X}", (offBlock << sizeBlock) + 512));
    throw YGP::ParseError(error);
  }
}
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
ParseMSOffice::BAT ParseMSOffice::readBAT(std::istream &stream,
                                          const char *pBATBlocks,
                                          unsigned int cBlocks,
                                          unsigned int sizeBlock) {
  TRACE9("ParseMSOffice::readBAT (std::istream&, const char*, 2x unsigned) - "
         << cBlocks);

  BAT bat((cBlocks << sizeBlock) / sizeof(BAT::value_type));
  for (unsigned int i(0); i < cBlocks; ++i)
    readBlock(stream, YGP::get4BytesLSB(pBATBlocks + (i << 2)),
              reinterpret_cast<char *>(bat.data()) + (i << sizeBlock),
              sizeBlock);
  return bat;
}
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
std::vector<char> ParseMSOffice::readFile(std::istream &stream,
                                          unsigned int offBlock, const BAT &bat,
                                          unsigned int blocks,
                                          unsigned int sizeBlock) {
...
/// \throw YGP::ParseError In case of error a describing text
//-----------------------------------------------------------------------------
unsigned int ParseMSOffice::getBlock(const BAT &bat, unsigned int start,
                                     unsigned int nr) {
  ...
  TRACE8("ParseMSOffice::getBlock (const BAT&, 2x unsigned int) - Result: "
         << start);
  return start;
}
