...
// CREATED     : 20.03.2005
// COPYRIGHT   : Copyright (C) 2005 - 2008, 2011, 2024, 2026
...
void ParseOGG::parse(std::istream& stream, Properties& result) {
    ...
    auto ogg = x3::lit("OggS") >> x3::repeat(0x50)[x3::byte_] >> x3::byte_[setLength] >> skip(length) // Segment table
               >> x3::lit("\x03vorbis") >> x3::little_dword[setLength] >> skip(length)                // Vendor string
               >> x3::little_dword[setCount] >> SpiritParser::times(count)[x3::little_dword[setLength] >> bytes(length)[comment]];
