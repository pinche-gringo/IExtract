...
void ParseOGG::parse(std::istream &stream, Properties &result) {
  ...
  auto ogg = x3::lit("OggS") >> x3::repeat(0x50)[x3::byte_] >>
             ...
             >> x3::lit("\x03vorbis") >> x3::little_dword[setLength] >>
             skip(length) // Vendor string
             >> x3::little_dword[setCount] >>
             SpiritParser::times(
                 count)[x3::little_dword[setLength] >> bytes(length)[comment]];
...
void ParseOGG::foundComment(const std::string &comment, Properties &result) {
  TRACE8("ParseOGG::foundComment (const std::string&, Properties&) - "
         << comment);
