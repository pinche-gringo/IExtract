...
static constexpr const char *DEFAULT_XML_FORMAT =
    "<tr><td>%n</td><td>%t</td>"
...
private:
  ...
  struct FileFunction : public YGP::File {
    FileFunction(const YGP::File &file, HANDLER fnc)
        : YGP::File(file), fnc(fnc) {}
  ...
  std::condition_variable_any
      cvListFiles; ///< Signals changes of listFiles or cActive
...
const YGP::IVIOApplication::longOptions Application::lo[] = {
    {IVIOAPPL_HELP_OPTION}, {"recursive", 'r'},   {"format", 'F'},
    {"title", 'T'},
#ifdef ENABLE_THREADS
    {"threads", 't'},
#endif
    {"include", 'i'},       {"exclude", 'x'},     {"show-errors", 'e'},
    {"separate", 's'},      {"add-unknown", 'u'}, {"new", 'n'},
    {"ini-file", 'f'},      {"version", 'V'},     {"output", 'o'},
    {"append", 'a'},        {"app-file", 'A'},    {"prepend", 'p'},
    {"pre-file", 'P'},      {"sort", 'S'},        {"mode", 'M'},
    {"list-types", 'l'},    {nullptr, '\0'}};
...
void Application::showHelp() const {
  std::cout
      << _("Extracts descriptions/comments out of files (depending on the "
           "file-type)\n\nUsage:")
      ...
      << "\n  -F, --format=FORMAT ... " << _("Format of output; default: ")
      << DEFAULT_FORMAT << "\n  -T, --title=TITLE ..... "
      << _("Title of output") << "\n  -s, --separate=TEXT ... "
...
#ifdef ENABLE_THREADS
      << "\n  -t, --threads=NR ...... "
      << _("Number of threads for examining files (default: 1)")
#endif
      << "\n  -n, --new=DAYS:TEXT ... "
      << _("Show TEXT for files younger than DAYS days (def: 30)")
      << "\n  -i, --include=LIST .... " << _("Files to inspect")
      << "\n  -x, --exclude=LIST .... " << _("Files not to inspect")
      << "\n  -f, --ini-file=FILE ... "
      << _("Read further options from specified file")
...
bool Application::handleOption(const char option) {
  ...
  case 's': {
    ...
    if (pSep)
      ...
      std::string error(
          _("-warning: Option `%1' needs an argument! Ignoring option!\n"));
  ...
  case 'o': {
    ...
    std::optional<OutputStyle> style(pType ? getOutputStyle(pType)
                                           : std::nullopt);
    outputStyle = style.value_or(OutputStyle::TEXT);
    if (!style) {
      std::string error(
          _("-warning: Style of output `%1' is not valid! Using text\n"));
  ...
  case 't': {
    ...
    auto [end, rc](std::from_chars(threads.data(),
                                   threads.data() + threads.size(), count));
    if ((rc != std::errc()) || (end != (threads.data() + threads.size())) ||
        !count)
  ...
  case 'n': {
    ...
    unsigned int time(0);
    const char *pos(
        std::from_chars(value.data(), value.data() + value.size(), time).ptr);
  ...
  case 'i': {
    ...
    if (files) {
      ...
      tokenizer list(lFiles,
                     boost::char_separator<char>(YGP::Path::SEPARATOR_STR));
  ...
  case 'A': {
    ...
    if (file) {
      ...
      if (!input) {
        std::string error(_(
            "-error: `%1' is not a (readable) file! Ignoring option `%2'!\n"));
  ...
  case 'M':
    if (checkOptionValue()) {
      ...
      if (setMode(mode)) {
        std::string error(
            _("-warning: Invalid mode `%1'! Ignoring option 'M'!\n"));
...
//-----------------------------------------------------------------------------
std::optional<Application::OutputStyle>
Application::getOutputStyle(std::string_view style) {
...
int Application::perform(int argc, const char *argv[]) {
  ...
  using CreateWriter = std::unique_ptr<Writer> (*)(
      const std::string &, const std::string &, unsigned long);

  // This declaration must be in the same order as the OutputStyle enum
  static constexpr CreateWriter fnc[] = {
      &TextWriter::create, &QuotedTextWriter::create, &HTMLWriter::create,
...
void Application::handleFiles(const char *pFile) {
  ...
  if (iniOpts.sort)
    search =
        std::make_unique<YGP::SortedDirSearch<YGP::ExtDirectorySearch>>(pFile);
  else
    search = std::make_unique<YGP::ExtDirectorySearch>(pFile);
  YGP::ExtDirectorySearch &ds(*search);

  tokenizer list(filelist,
                 boost::char_separator<char>(YGP::Path::SEPARATOR_STR));

  for (const auto &entry : list)
    (entry.at(0) == 'i') ? ds.addFilesToInclude(entry.substr(1))
  ...
  while (file) {
    ...
    if (fnc) {
#ifdef ENABLE_THREADS
      {
        ...
        cvListFiles.wait(
            lock, [this] { return listFiles.size() < MAX_PENDING_FILES; });
      ...
      if (threads.size() < cThreads)
        threads.emplace_back(
            [this](std::stop_token stop) { processThread(stop); });
#else
  ...
  if (options & RECURSIVE) {
    ...
    file =
        ds.find(ds.getDirectory() + "*", YGP::DirectorySearch::FILE_DIRECTORY);
    while (file) {
      if (!YGP::IDirectorySearch::isSpecial(file->name())) {
        {
          std::scoped_lock lock(mxOutput);
          writer->printSeparator(std::cout, *file, iniOpts.separate,
                                 iniOpts.title);
        }
...
void Application::processThread(std::stop_token stop) {
  while (true) {
    ...
    listFiles.pop();
    ++cActive;
    TRACE1("Application::processThread (std::stop_token) - File "
           << file.name() << "; Remaining: " << listFiles.size());
    lock.unlock();
    cvListFiles.notify_all();
...
void Application::convertFromWideChar(Properties &prop) {
  for (auto member : {&Properties::strTitle, &Properties::strComment,
                      &Properties::strAuthor}) {
    std::string &value(prop.*member);
    if ((value.size() > 1) &&
        std::iscntrl(static_cast<unsigned char>(value[1])) &&
        !(value.size() & 1)) {
...
void Application::setPlugins() {
  TRACE1("Application::setPlugins (std::map<std::string, std::string>&): "
         << dynHandlers.size());

  using MatchFunction =
      std::remove_pointer_t<YGP::FileTypeCheckerByContent::MATCHFNC>;
  ...
  for (const auto &[extension, library] : dynHandlers) {
    try {
      ...
      auto *byContent(
          dynamic_cast<YGP::FileTypeCheckerByContent *>(ftchk.get()));
      if (!plugin.has(PLUGIN_PROCESS) ||
          (byContent && !plugin.has(PLUGIN_CHECKTYPE))) {
      ...
      // Add handling method
      if (byContent)
        byContent->addType(offset,
                           &plugin.get<MatchFunction>(PLUGIN_CHECKTYPE));
      else
        dynamic_cast<YGP::FileTypeCheckerByExtension &>(*ftchk).addType(
            extension.c_str(), offset);
      handlers[offset++] =
          &plugin.get<std::remove_pointer_t<HANDLER>>(PLUGIN_PROCESS);
